    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                float normalizedY = GetNormalizedDist(o.worldPos.y);
                float3 localFlyDirection = normalize(mul(unity_WorldToObject, _FlyDirection.xyz));

                float flyDegree = (_Threshold - normalizedY) / _EdgeWidth;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
				...
				float cutout = lerp(commonNoise,whiteNoise,_DistanceEffect);
